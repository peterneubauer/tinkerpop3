                ...
                if (fieldName.equals(GraphSONTokens.VARIABLES)) {
                    ...
                    while (parser.nextToken() != JsonToken.END_ARRAY) {
                        ...
                        readVertexData(vertexData, detachedVertex -> {
                            final Iterator<Vertex> itty = graph.v(detachedVertex.id());
                            final Vertex v = itty.hasNext() ? itty.next() : graph.addVertex(T.label, detachedVertex.label(), T.id, detachedVertex.id());
