...
public class PageRankStep extends AbstractStep<Vertex, Pair<Vertex, Double>> {
    ...
    public Traverser<Pair<Vertex, Double>> processNextStart() {
        ...
        return traverser.makeChild(this.getLabel(), new Pair<>(vertex, (Double) this.resultantGraph.iterators().vertexIterator(vertex.id()).next().value(PageRankVertexProgram.PAGE_RANK)));
    }
}
